...
import { describeCoord, fmtVec, sameCell, type PlanningMode } from '../reactor/types'
...
export const ControlPanel = () => {
  ...
  const { run, pause, step, reset, setEntry, setMode, toggleFailed } = useCommands()
  if (!scenario || !sim) return null

  const { plan, stepIndex, isRunning, showFailed, failureReason, entry, mode } = sim
  const atEnd = stepIndex >= plan.path.length - 1
  const lastIndex = Math.max(plan.path.length - 1, 0)

  return (
    <div className="space-y-4">
      <Panel title="Scenario" icon={Target}>
        <p className="text-[13px] font-semibold text-slate-100">{scenario.title}</p>
        <p className="mt-1.5 text-[12px] leading-relaxed text-slate-400">{scenario.context}</p>
        <dl className="mt-3 grid grid-cols-2 gap-2 text-[11px]">
          <div className="rounded-lg border border-slate-700/50 bg-slate-900/40 px-2.5 py-2">
            ...
            <dd className="mt-0.5 font-mono text-[12px] text-emerald-300">{fmtVec(scenario.target)}</dd>
          </div>
          <div className="rounded-lg border border-slate-700/50 bg-slate-900/40 px-2.5 py-2">
      ...
      <Panel title="Simulation controls" icon={Play}>
        ...
        <p className="mt-2.5 text-[11px] text-slate-500">
          Step {Math.min(stepIndex, lastIndex)} of {lastIndex}
        </p>
      </Panel>
        ...
        <div className="space-y-2">
          {scenario.entries.map((candidate) => {
            ...
            return (
              ...
              >
                <span className="font-mono text-[12px]">{fmtVec(candidate)}</span>
                <span className="text-[11px] text-slate-400">{describeCoord(candidate)}</span>
              </button>
            )
          })}
        </div>
        <p className="mt-2 text-[11px] text-slate-500">
          Entry corridors can also be clicked directly in the streamed scene.
        </p>
      </Panel>
      ...
      <Panel title="Failed candidate" icon={AlertTriangle}>
        ...
        >
          <span className="text-[12px] text-slate-300">Show unsafe direct route</span>
          <span
